...
mol = MolChoose('A01', 'B05', DataSource='data/r_group_decomp.csv')
...
class MyGame(arcade.Window):
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        ...
        if len(clicked) > 0: # checks a button has been clicked
            choice = clicked[0]
            # checks if the button is for an assa
            ...
            # checks if the button is an action button
                ...
                if choice.button == 'run_assays': # shows the results of the selected assays
                    if self.assay_results == []:
                        pass
                    else:
                        self.assay_results_print = self.assay_results # adds the results to another list to print
                        [b._set_color(arcade.color.WHITE) for b in self.button_list] # changes buttons back to white
                elif choice.button == 'clear_choices': # clears the selected assays and recorded data
                    [b._set_color(arcade.color.WHITE) for b in self.button_list] # changes buttons back to white
                    self.assay_results_print = []
                    self.assay_results = []
                    self.total_cost = 0
